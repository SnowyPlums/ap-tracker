  ...
  return <div className="page-shell">
    ...
    <main className="main">
      ...
      <section className="cards">{filteredRooms.map((room) => <Link className="card" href={"/rooms/" + room.room_key} key={room.room_key}>
        ...
        <p className="muted"><button className="connection-toggle" type="button" title={visibleConnections.has(room.room_key) ? "Click to hide connection" : "Click to show connection"} aria-label={visibleConnections.has(room.room_key) ? "Hide room connection" : "Show room connection"} onClick={(event) => { event.preventDefault(); event.stopPropagation(); setVisibleConnections((current) => { const next = new Set(current); if (next.has(room.room_key)) next.delete(room.room_key); else next.add(room.room_key); return next; }); }}>{visibleConnections.has(room.room_key) ? room.host + ":" + room.port : "Connection hidden"}</button></p>
