...
function SlotBox({ slot, box, itemNames, onAction, locationHref }: {
  ...
  if (box === "hints") return <div className="box"><h3>Hints</h3><div className="box-content">
    <input className="input hint-input" list={"items-" + slot.id} placeholder="Search for an item" value={hint} onChange={(event) => setHint(event.target.value)} />
    <datalist id={"items-" + slot.id}>{itemNames.map((name) => <option value={name} key={name} />)}</datalist>
