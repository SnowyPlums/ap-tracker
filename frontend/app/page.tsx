  ...
  return <div className="page-shell">
    ...
    <main className="main">
      ...
      <div className="filters"><input className="input" placeholder="Search games" value={search} onChange={(e) => setSearch(e.target.value)} />
        <select className="select" value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">All statuses</option><option value="in_progress">In progress</option><option value="completed">Completed</option><option value="canceled">Canceled</option></select>
      </div>
      <section className="cards">{filteredRooms.map((room) => <Link className="card" href={"/rooms/" + room.room_key} key={room.room_key}>
        <div className="card-head"><h2>{room.label}</h2><span className={"tag " + room.game_status}>{statusLabel(room.game_status)}</span></div>
        <p className="muted masked-connection" role="button" tabIndex={0} onClick={(event) => { event.preventDefault(); event.stopPropagation(); setVisibleConnections((current) => { const next = new Set(current); if (next.has(room.room_key)) next.delete(room.room_key); else next.add(room.room_key); return next; }); }}>{visibleConnections.has(room.room_key) ? room.host + ":" + room.port : "Connection hidden"}</p>
